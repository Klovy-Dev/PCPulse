...
import { Gamepad2, RefreshCw, Search, Play, Gauge, History, AlertTriangle, Clock, Zap, Power, CheckCircle } from "lucide-react";
...
export default function GamesTab({ userId }: Props) {
  ...
  // ── Auto-Boost ──
  const [autoEnabled,   setAutoEnabled]   = useState(() => localStorage.getItem("nexboost_autoboost") === "true");
  ...
  const BOOST_INTERVAL = 90; // secondes
  ...
  const runAutoBoost = async () => {
    try {
      ...
      if (result.game_detected) {
        setBoostLog(prev => [
          { time: new Date().toLocaleTimeString("fr-FR"), result },
          ...prev.slice(0, 9),
        ]);
      }
    } catch {}
  ...
  useEffect(() => {
    localStorage.setItem("nexboost_autoboost", String(autoEnabled));
  ...
  const handleBoostGame = async (game: InstalledGame) => {
    ...
    try {
      ...
      setBoostLog(prev => [{
        ...
        result: { game_detected: count > 0, game_name: game.name, processes_boosted: count },
      }, ...prev.slice(0, 9)]);
    } catch {}
    setBoosting(null);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }} className="animate-fadeIn">
      ...
      <div style={{ padding: "20px 22px 0", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 16 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9", lineHeight: 1.2, margin: 0 }}>Jeux</h1>
            <p style={{ fontSize: 11, color: "#4b5563", marginTop: 5, marginBottom: 0 }}>
              Bibliothèque Steam & Epic · Auto-Boost · Benchmark
            </p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
            {/* Indicateur Auto-Boost actif */}
            {autoEnabled && (
              <div style={{
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: "#4ade80" }}>AUTO {nextBoostIn}s</span>
              </div>
            )}
            <div style={{
              ...
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#4b5563" }}>JEUX</span>
              <span style={{ fontSize: 28, fontWeight: 800, fontFamily: "monospace", color: "#a78bfa", lineHeight: 1 }}>{games.length}</span>
              <span style={{ fontSize: 9, color: "#4b5563" }}>{totalGb > 0 ? `${totalGb.toFixed(0)} GB` : "détectés"}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <button onClick={() => setInner("autoboost")} style={innerTabStyle(inner === "autoboost")}>
            <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
              {autoEnabled && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4ade80", display: "inline-block" }} />}
              Auto-Boost
            </span>
          </button>
        ...
        <div style={{ padding: "16px 22px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                {[
                  ...
                  <div key={s.label} style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "12px 14px", textAlign: "center" }}>
              ...
              {loading ? (
                ...
                <div style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "48px 20px", textAlign: "center" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  {filtered.length === 0 && search && (
                    <div style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "24px 20px", textAlign: "center" }}>
          ...
          {inner === "autoboost" && (
            <>
              ...
              <div style={{
                background: "#0c0c1a",
                border: `1px solid ${autoEnabled ? "rgba(74,222,128,0.3)" : "rgba(255,255,255,0.06)"}`,
                borderRadius: 12, padding: "18px 20px",
                position: "relative", overflow: "hidden",
              }}>
                {autoEnabled && (
                  <div style={{ position: "absolute", top: -30, right: -30, width: 120, height: 120, background: "radial-gradient(circle, rgba(74,222,128,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />
                )}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{
                      ...
                      background: autoEnabled ? "rgba(74,222,128,0.12)" : "rgba(255,255,255,0.05)",
                      border: `1px solid ${autoEnabled ? "rgba(74,222,128,0.3)" : "rgba(255,255,255,0.08)"}`,
                    }}>
                      <Zap size={20} style={{ color: autoEnabled ? "#4ade80" : "#4b5563" }} />
                    </div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#f1f5f9" }}>Mode Gaming Auto</div>
                      <div style={{ fontSize: 11, color: "#4b5563", marginTop: 2 }}>
                        Détecte les jeux et booste toutes les {BOOST_INTERVAL}s — libère la RAM + priorité CPU
                      </div>
                    </div>
                  </div>
                  <button
                    ...
                    style={{
                      padding: "9px 20px", borderRadius: 9, fontSize: 12, fontWeight: 700, flexShrink: 0,
                      cursor: "pointer", transition: "all 0.15s",
                      background: autoEnabled ? "rgba(248,113,113,0.12)" : "rgba(74,222,128,0.12)",
                      border: `1px solid ${autoEnabled ? "rgba(248,113,113,0.3)" : "rgba(74,222,128,0.3)"}`,
                      color: autoEnabled ? "#f87171" : "#4ade80",
                    }}
                  >
                ...
                {/* Barre de progression avant le prochain boost */}
                {autoEnabled && (
                  <div style={{ marginTop: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 10, color: "#4b5563" }}>Prochain boost</span>
                      <span style={{ fontSize: 10, fontWeight: 600, fontFamily: "monospace", color: "#4ade80" }}>{nextBoostIn}s</span>
                    </div>
                    <div style={{ height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 2, overflow: "hidden" }}>
                      <div style={{
                        height: "100%",
                        width: `${((BOOST_INTERVAL - nextBoostIn) / BOOST_INTERVAL) * 100}%`,
                        background: "#4ade80", borderRadius: 2, transition: "width 1s linear",
                      }} />
                    </div>
                  </div>
                )}
                ...
                <div style={{
                  ...
                  {lastBoost.game_detected ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 9, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.25)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <CheckCircle size={16} style={{ color: "#4ade80" }} />
                      </div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#f1f5f9" }}>{lastBoost.game_name}</div>
                        <div style={{ fontSize: 11, color: "#4b5563", marginTop: 2 }}>
                          {lastBoost.processes_boosted} processus boostés · RAM libérée
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 12, color: "#4b5563" }}>Aucun jeu détecté — en attente...</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <button
                ...
                style={{
                  width: "100%", padding: "12px 20px", borderRadius: 9, fontSize: 13, fontWeight: 700,
                  background: "rgba(56,189,248,0.08)", border: "1px solid rgba(56,189,248,0.25)", color: "#38bdf8",
              ...
              {/* Log */}
              {boostLog.length > 0 && (
                <div style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 18px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {boostLog.map((entry, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 7, background: "rgba(255,255,255,0.02)" }}>
              ...
              {/* Info */}
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "12px 16px", borderRadius: 8, background: "rgba(56,189,248,0.06)", border: "1px solid rgba(56,189,248,0.15)" }}>
                <AlertTriangle size={12} style={{ color: "#38bdf8", marginTop: 1, flexShrink: 0 }} />
                <p style={{ fontSize: 11, color: "#38bdf8", lineHeight: 1.6, margin: 0 }}>
                  Détecte automatiquement Steam, Epic, Riot, Battle.net et les jeux courants. Lance un nettoyage RAM + passage en priorité haute toutes les {BOOST_INTERVAL}s.
                </p>
              </div>
            </>
          )}
            ...
            <>
              <div style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "24px 20px" }}>
                {!benchRunning && !benchResult && (
                  <div style={{ textAlign: "center", padding: "16px 0" }}>
                    <div style={{ width: 64, height: 64, borderRadius: 14, margin: "0 auto 20px", background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Gauge size={28} style={{ color: "#fbbf24" }} />
                    </div>
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9", margin: "0 0 8px" }}>Benchmark système</h3>
                    <p style={{ fontSize: 12, color: "#4b5563", margin: "0 auto 24px", maxWidth: 380, lineHeight: 1.6 }}>Mesure les performances brutes CPU, RAM et disque. Durée : 3 à 5 secondes.</p>
                    <button onClick={handleRunBench} style={{ padding: "10px 28px", borderRadius: 8, fontSize: 13, fontWeight: 700, background: "#38bdf8", color: "#020817", border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 8, transition: "all 0.15s" }} onMouseEnter={e => { e.currentTarget.style.background = "#7dd3fc"; }} onMouseLeave={e => { e.currentTarget.style.background = "#38bdf8"; }}>
                ...
                {benchRunning && (
                  <div style={{ textAlign: "center", padding: "16px 0" }}>
                    <div style={{ width: 64, height: 64, borderRadius: 14, margin: "0 auto 20px", background: "rgba(56,189,248,0.1)", border: "1px solid rgba(56,189,248,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                {!benchRunning && benchResult && (
                  <>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, padding: "0 4px" }}>
                      <span style={{ fontSize: 11, color: "#4b5563" }}>Durée : {(benchResult.duration_ms / 1000).toFixed(1)}s</span>
                    ...
                    <button onClick={handleRunBench} style={{ width: "100%", padding: "10px 20px", borderRadius: 8, fontSize: 13, fontWeight: 600, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, transition: "all 0.15s" }} onMouseEnter={e => { e.currentTarget.style.background = "rgba(56,189,248,0.06)"; e.currentTarget.style.borderColor = "rgba(56,189,248,0.25)"; e.currentTarget.style.color = "#38bdf8"; }} onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.04)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.color = "#94a3b8"; }}>
              ...
              {history.length > 0 && (
                <div style={{ background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "16px 18px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {history.map((h, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            {[{ l: "CPU", v: h.cpu_score }, { l: "RAM", v: h.ram_score }, { l: "Disk", v: h.disk_score }].map(s => (
                              <span key={s.l} style={{ fontSize: 10, color: "#4b5563" }}>{s.l} <span style={{ fontWeight: 700, fontFamily: "monospace", color: scoreColor(s.v) }}>{s.v}</span></span>
                            ))}
                          </div>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "12px 16px", borderRadius: 8, background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)" }}>
                <AlertTriangle size={12} style={{ color: "#fbbf24", marginTop: 1, flexShrink: 0 }} />
                <p style={{ fontSize: 11, color: "#fbbf24", lineHeight: 1.6, margin: 0 }}>Fermez les applications gourmandes avant de lancer le test pour des résultats représentatifs.</p>
              </div>
            </>
          )}
        </div>
...
/* ── Ligne de jeu ── */
function GameRow({ game, onOpen, onBoost, boosting }: {
  game: InstalledGame;
  onOpen:  (path: string) => void;
  onBoost: (game: InstalledGame) => void;
  boosting: boolean;
}) {
  const color = game.platform === "Steam" ? "#60a5fa" : "#a78bfa";
  return (
    <div
      style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "#0c0c1a", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, transition: "all 0.15s" }}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: "#f1f5f9", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{game.name}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        ...
        <button
          ...
          disabled={boosting}
          title="Booster la priorité CPU du processus"
          style={{
            padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600,
            background: boosting ? "rgba(74,222,128,0.08)" : "rgba(74,222,128,0.06)",
            border: `1px solid ${boosting ? "rgba(74,222,128,0.4)" : "rgba(74,222,128,0.2)"}`,
            color: "#4ade80", cursor: boosting ? "not-allowed" : "pointer",
            display: "flex", alignItems: "center", gap: 5, transition: "all 0.15s",
            opacity: boosting ? 0.6 : 1,
          }}
          onMouseEnter={e => { if (!boosting) e.currentTarget.style.background = "rgba(74,222,128,0.15)"; }}
          onMouseLeave={e => { e.currentTarget.style.background = "rgba(74,222,128,0.06)"; }}
        >
          {boosting
            ? <div className="animate-spin" style={{ width: 10, height: 10, borderRadius: "50%", border: "1.5px solid rgba(74,222,128,0.2)", borderTopColor: "#4ade80" }} />
            : <Power size={10} />}
          Boost
        </button>
        {/* Ouvrir dossier */}
        {game.install_path && (
          <button
            ...
            style={{ padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, transition: "all 0.15s" }}
